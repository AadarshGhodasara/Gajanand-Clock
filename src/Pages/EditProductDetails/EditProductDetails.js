    ...
    if (productName || productDetail || productPrice || isProductImageChange) {
      if (isProductImageChange) {
        if (types.includes(productFile.type)) {
          ...
          image.delete();
          await storage
            .ref(`${productType}/${productFile.name}`)
            .put(productFile);
          await storage
            .ref(`${productType}`)
            .child(productFile.name)
            .getDownloadURL()
            .then((imgUrl) => {
        ...
        <div className="w-75 mx-auto shadow p-5">
          ...
          {productEnable ? (
            <button
              ...
              onClick={() => enableOrDisableProduct(0)}
            >
              Disable Product In Store
            </button>
          ) : (
            <button
              className="btn btn-primary btn-block"
              onClick={() => enableOrDisableProduct(1)}
            >
              Enable Product In Store
            </button>
          )}
        </div>
